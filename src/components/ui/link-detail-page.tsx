...
import { Power, Play, Link, Calendar, Globe, MousePointerClick, Activity, Copy, Lock, Trash2, Loader2, MapPin, RefreshCw, Clock, Monitor, Smartphone, Laptop, ArrowLeft } from "lucide-react";
...
const getDeviceDetails = (userAgent?: string) => {
  ...
  if (ua.includes("iphone") || ua.includes("ipad") || ua.includes("ipod")) {
    return { name: "iOS", icon: Smartphone };
  }
...
export function LinkDetailPage({ linkId, onBack }: LinkDetailPageProps) {
  ...
  const fetchClickStats = async (urlId: string) => {
    ...
    try {
      const { data, error } = await supabase
        .from('clicks')
        .select('id, country, city, clicked_at, ip_address, user_agent')
        .eq('url_id', urlId)
          ...
          {isLoadingStats ? (
            ...
            <div className="max-h-[400px] overflow-y-auto overflow-x-auto space-y-1">
              <div className="min-w-[600px]">
                ...
                {clickLog.map((entry) => (
                    <div className="grid grid-cols-12 gap-2 items-center text-sm px-3 py-2.5 rounded-lg hover:bg-muted/20 transition-colors">
                      ...
                      <div className="col-span-2 flex justify-end items-center gap-1.5 min-w-0">
                        {(() => {
                          const device = getDeviceDetails(entry.user_agent);
